...
func TestPublicEvent(t *testing.T) {
	...
	err := json.Unmarshal([]byte(event), &publicEvent)
	if err != nil {
		t.Error(err)
	} else {
		repo := publicEvent.Repository
		if repo.Id != 35129377 {
			t.Error("publicEvent.Repository.Id != 35129377")
		}
		sender := publicEvent.Sender
		if sender.Login != "baxterthehacker" {
			t.Error("publicEvent.Sender.Login != baxterthehacker")
		}
	}
}
