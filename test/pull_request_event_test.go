...
func TestPullRequestEvent(t *testing.T) {
	...
	err := json.Unmarshal([]byte(event), &pullRequestEvent)
	if err != nil {
		t.Error(err)
	} else {
		if pullRequestEvent.Action != "opened" {
			t.Error("pullRequestEvent.Action != opened")
		}
		if pullRequestEvent.Number != 1 {
			t.Error("pullRequestEvent.Number != 1")
		}
		if pullRequestEvent.PullRequest.Id != 34778301 {
			t.Error("pullRequestEvent.PullRequest.Id != 34778301")
		}
		if pullRequestEvent.PullRequest.Head.Ref != "changes" {
			t.Error("pullRequestEvent.PullRequest.Head.Id != changes")
		}
		if pullRequestEvent.PullRequest.XLinks.Self.Href != "https://api.github.com/repos/baxterthehacker/public-repo/pulls/1" {
			t.Error("pullRequestEvent.PullRequest.XLinks.Self.Href != https://api.github.com/repos/baxterthehacker/public-repo/pulls/1")
		}
		repo := pullRequestEvent.Repository
		if repo.Id != 35129377 {
			t.Error("pullRequestEvent.Repository.Id != 35129377")
		}
		sender := pullRequestEvent.Sender
		if sender.Login != "baxterthehacker" {
			t.Error("pullRequestEvent.Sender.Login != baxterthehacker")
		}
	}
}
