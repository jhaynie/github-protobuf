...
func TestPullRequestReviewCommentEvent(t *testing.T) {
	...
	err := json.Unmarshal([]byte(event), &pullRequestReviewCommentEvent)
	if err != nil {
		t.Error(err)
	} else {
		if pullRequestReviewCommentEvent.Action != "created" {
			t.Error("pullRequestReviewCommentEvent.Action != created")
		}
		if pullRequestReviewCommentEvent.Comment.DiffHunk != "@@ -1 +1 @@\n-# public-repo" {
			t.Error("pullRequestReviewCommentEvent.Comment.DiffHunk != @@ -1 +1 @@\n-# public-repo")
		}
		if pullRequestReviewCommentEvent.PullRequest.Locked {
			t.Error("pullRequestReviewCommentEvent.PullRequest.Locked should have been false")
		}
		repo := pullRequestReviewCommentEvent.Repository
		if repo.Id != 35129377 {
			t.Error("pullRequestReviewCommentEvent.Repository.Id != 35129377")
		}
		sender := pullRequestReviewCommentEvent.Sender
		if sender.Login != "baxterthehacker" {
			t.Error("pullRequestReviewCommentEvent.Sender.Login != baxterthehacker")
		}
	}
}
