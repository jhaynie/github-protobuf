...
func TestGollumEvent(t *testing.T) {
	...
	err := json.Unmarshal([]byte(event), &gollumEvent)
	if err != nil {
		t.Error(err)
	} else {
		if len(gollumEvent.Pages) != 1 {
			t.Error("len(gollumEvent.Pages) != 1")
		}
		page := gollumEvent.Pages[0]
		if page.PageName != "Home" {
			t.Error("gollumEvent.Pages[0].PageName != Home")
		}
		repo := gollumEvent.Repository
		if repo.Id != 35129377 {
			t.Error("gollumEvent.Repository.Id != 35129377")
		}
		if repo.Owner.Login != "baxterthehacker" {
			t.Error("gollumEvent.Repository.Owner.Login != baxterthehacker")
		}
		sender := gollumEvent.Sender
		if sender.Login != "jasonrudolph" {
			t.Error("gollumEvent.Sender.Login != jasonrudolph")
		}
	}
}
