...
func TestRepositoryEvent(t *testing.T) {
	...
	var repositoryEvent RepositoryEvent
	err := json.Unmarshal([]byte(event), &repositoryEvent)
	if err != nil {
		t.Error(err)
	} else {
		if repositoryEvent.Action != "created" {
			t.Error("repositoryEvent.Action != created")
		}
		if repositoryEvent.Organization.Id != 7649605 {
			t.Error("repositoryEvent.Organization.Id != 7649605")
		}
		repo := repositoryEvent.Repository
		if repo.Id != 27496774 {
			t.Error("repositoryEvent.Repository.Id != 27496774")
		}
		sender := repositoryEvent.Sender
		if sender.Login != "baxterthehacker" {
			t.Error("repositoryEvent.Sender.Login != baxterthehacker")
		}
	}
}
