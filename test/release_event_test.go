...
func TestReleaseEvent(t *testing.T) {
	...
	err := json.Unmarshal([]byte(event), &releaseEvent)
	if err != nil {
		t.Error(err)
	} else {
		if releaseEvent.Action != "published" {
			t.Error("releaseEvent.Action != published")
		}
		if releaseEvent.Release.Id != 1261438 {
			t.Error("releaseEvent.Release.Id  != 1261438")
		}
		repo := releaseEvent.Repository
		if repo.Id != 35129377 {
			t.Error("releaseEvent.Repository.Id != 35129377")
		}
		sender := releaseEvent.Sender
		if sender.Login != "baxterthehacker" {
			t.Error("releaseEvent.Sender.Login != baxterthehacker")
		}
	}
}
