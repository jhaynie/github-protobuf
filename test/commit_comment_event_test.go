...
func TestCommitCommentEvent(t *testing.T) {
	...
	err := json.Unmarshal([]byte(event), &commentEvent)
	if err != nil {
		t.Error(err)
	} else {
		if commentEvent.Action != "created" {
			t.Error("commentEvent.Action != created")
		}
		comment := commentEvent.Comment
		if comment.Id != 11056394 {
			t.Error("commentEvent.Comment.Id != 11056394")
		}
		comment_user := comment.User
		if comment_user.Id != 6752317 {
			t.Error("commentEvent.Comment.User.Id != 6752317")
		}
		repo := commentEvent.Repository
		if repo.Id != 35129377 {
			t.Error("commentEvent.Repository.Id != 35129377")
		}
		if repo.Owner.Login != "baxterthehacker" {
			t.Error("commentEvent.Repository.Owner.Login != baxterthehacker")
		}
		sender := commentEvent.Sender
		if sender.Login != "baxterthehacker" {
			t.Error("commentEvent.Sender.Login != baxterthehacker")
		}
	}
}
