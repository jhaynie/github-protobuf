...
func TestPullRequestReviewEvent(t *testing.T) {
	...
	err := json.Unmarshal([]byte(event), &pullRequestReviewEvent)
	if err != nil {
		t.Error(err)
	} else {
		if pullRequestReviewEvent.Action != "submitted" {
			t.Error("pullRequestReviewEvent.Action != submitted")
		}
		if pullRequestReviewEvent.Review.Id != 2626884 {
			t.Error("pullRequestReviewEvent.Review.Id != 2626884")
		}
		if pullRequestReviewEvent.PullRequest.Id != 87811438 {
			t.Error("pullRequestReviewEvent.PullRequest.Id != 87811438")
		}
		repo := pullRequestReviewEvent.Repository
		if repo.Id != 35129377 {
			t.Error("pullRequestReviewEvent.Repository.Id != 35129377")
		}
		sender := pullRequestReviewEvent.Sender
		if sender.Login != "baxterthehacker" {
			t.Error("pullRequestReviewEvent.Sender.Login != baxterthehacker")
		}
	}
}
